...
dateDetected = cell(imageCount, 1);
fileName = cell(imageCount, 1);
recall = zeros(imageCount, 1);

%% Run Script
parfor val = 1:imageCount
    %% Read image
    ...
    [~, recall(val)] = bboxPrecisionRecall(expandedFilteredTextROI, gTruth.LabelData{val, 1}{1});
    ...
    fileName{val, 1} = string(gTruth.DataSource.Source{val});
    dateDetected{val, 1} = string(vertcat(validTextDate{:}, validTextYear{:}, validNumeric{:}));
end

detections = dateDetected;
known = dateLabels.Dates;
files = fileName;

known = known(recall > 0);
detections = detections(recall > 0);
fileName = fileName(recall > 0);
